...
import static org.inria.scale.streams.operators.SortBy.DESCENDING_ORDER;
...
public class SortByTest {
	...
	public void shouldSortInDescendingOrderWhenSpecified() throws Exception {
		...
		operator.setOrder(DESCENDING_ORDER);
	...
	@Test
	public void shouldSortByTheComponentAndOrderSpecified() throws Exception {
		final List<Tuple> tuples = Arrays.asList(tupleWith("a", 'b'), tupleWith("b", 'd'), tupleWith("c", 'a'),
				tupleWith("d", 'c'));
		...
		assertThat(processedTuples,
				contains(tupleWith("b", 'd'), tupleWith("d", 'c'), tupleWith("a", 'b'), tupleWith("c", 'a')));
	}
